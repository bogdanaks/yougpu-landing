export const siteConfig = {
	name: 'YouGPU',
	url: 'https://yougpu.ru',
	description:
		'Аренда GPU серверов с почасовой оплатой: NVIDIA A100, H100, H200, RTX 4090. Идеально для обучения нейросетей, ML и 3D-рендеринга.',
	ogImage: 'https://yougpu.ru/og-image.jpg',
	twitterHandle: '@yougpu',
	keywords:
		'аренда gpu, gpu сервер, nvidia a100, nvidia h100, машинное обучение, рендеринг, deep learning, обучение нейросетей, облачные вычисления, почасовая аренда gpu, yougpu, dedicated server gpu'
};
